...
int SPEED = 120;        //Analoge Werte geben die Geschwindigkeit zum geradeaus Fahren vor (0-255)
...
void loop() 
{
  ...
  if(frontSensor <= 35) //war vorher auf Wert 20 eingestellet (Reaktion zu langsam)
  {
    ...
    else if(rightSensor <= 25 && leftSensor > 25)
    {
      if(degree == 0)
      {
        degree += 90;
...
//        delay(1000);
  ...
  else if(frontSensor > 25 && leftSensor > 25 && rightSensor > 25)
  {
    if(degree == 90)
    {
      degree = 0;   
...
void randomTurn()
{
  ...
  else
  {
    ...
    // drehen nach rechts um 90 grad
    analogWrite(ENA, SpeedDrehen); //speed 0 - 255
    analogWrite(ENB, SpeedDrehen); //speed 0 - 255h
    digitalWrite(in1, LOW);
    digitalWrite(in2, HIGH);
    digitalWrite(in3, HIGH);
    digitalWrite(in4, LOW);
