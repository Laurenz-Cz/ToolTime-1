...
//--------------------Flame-------
#define FLAME 0 // connect DO pin of sensor to this pin
...
//int SPEED1 = 80;            //Analoge Werte geben die Geschwindigkeit zum geradeaus Fahren vor (0-255)
int SpeedDrehen = 120;      //Analoge Werte geben die Geschwindigkeit für das Drehen des Fahrzeugs vor (0-255)

int SpeedLinks = 122;
int SpeedRechts = 80;

int DELAY = 900;
int DELAYdoppelt = 1200;
int DELAYstehen = 2000;
int DELAYultra = 80;
...
void VorwaertsFahren (){
  analogWrite(ENA, SpeedLinks); //speed 0 - 255
  analogWrite(ENB, SpeedRechts); //speed 0 - 255
...
//--------------------Folgender Abschnitt: Wenn frontSensor && rightSensor auslöst----------------------------------------
      ...
      if (rightSensor <= dist2)//(frontSensor <= dist1 && rightSensor <= dist2)
      {
        //UltraschallSensoren();
        FrontRight();
      }
